...
public class CommandListener implements CommandExecutor {
    private String getHelpWithBalance(int i, boolean isOp) {
        ...
        res.add("&f");
        res.add(transText("&f \uE1C4 &f보유중인 잠수 포인트: &x&C&0&C&3&D&5[balance] &f\uE1C3".replace("[balance]", numberWithComma(i))));
        res.add("&f");
        if(isOp) {
            res.add("&f \uE1C4 &f/잠수포인트 [지급/차감/확인] (닉네임) (금액)");
        }
    ...
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args) {
        ...
        if( args.length == 2 ) {
            if( args[0].equalsIgnoreCase( "확인") ) {
                ...
                ArrayList<Map<String, Object>> dbRes2;
                try {
                    dbRes2 = Database.getInstance().execute("SELECT * FROM afk_point WHERE uuid=?", q2);
                ...
                ArrayList<String> res = new ArrayList<>();
                res.add("&f");
                res.add("&f \uE1C4 &f[nick] 님의 잠수 포인트: &x&C&0&C&3&D&5[balance] &f".replace("[balance]", numberWithComma(((Long) dbRes2.get(0).get("balance")).intValue())).replace("[nick]", args[1]));
                res.add("&f");
        ...
        if( args.length == 3 ) {
            ...
            if (args[0].equalsIgnoreCase("지급") && sender.isOp()) {
                ...
                try {
                    Database.getInstance().execute("UPDATE afk_point SET balance = balance + ? WHERE uuid = ? ", q3);
                } catch (SQLException e) {
                    throw new RuntimeException(e);
                }
                if (reciver.isOnline()) {
                    (reciver.getPlayer()).sendMessage(ChatColor.translateAlternateColorCodes('&', "&f \uE1C4 &f잠수 포인트 충전이 완료 되었습니다 &x&C&0&C&3&D&5+[amount] &f\uE1C3").replace("[nickname]", reciver.getName()).replace("[amount]", numberWithComma(parseInt(args[2]))));
                }
            ...
            if (args[0].equalsIgnoreCase("차감") && sender.isOp()) {
                ...
                if (reciver.isOnline()) {
                    (reciver.getPlayer()).sendMessage(ChatColor.translateAlternateColorCodes('&', "&f \uE1C4 &f관리자에 의해 잠수 포인트가 회수 되었습니다 &x&E&8&B&3&B&3-[amount] &f\uE1C3").replace("[nickname]", reciver.getName()).replace("[amount]", numberWithComma(parseInt(args[2]))));
                }
                sender.sendMessage("done");
                return false;
            }
        ...
        int balance =( (Long) dbres1.get(0).get("balance")).intValue();
